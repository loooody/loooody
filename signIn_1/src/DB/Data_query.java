...
public class Data_query {
	 ...
	 * 签到
	 */
	public void signOut(String userId,String signOut) {
		try {
			statement  = new DB_connect().DB_connect();
		//	String sqlstr = "insert into notice values("+name+","+context+")";
			int i = JOptionPane.showConfirmDialog(null, "签退?","sign",JOptionPane.YES_NO_OPTION);
			System.out.println(i+"\n");
			if(i == 0) {
			statement.executeUpdate("update sign set signOut_time = "+"'"+signOut+"'" 
                    +" where userId ="+"'"+userId+"'");
	 ...
	 * 签到情况
	 */
	public List<User> getAllSign_Employee(String id1,String date) throws ClassNotFoundException, SQLException{
		list = new ArrayList<User>();
		statement  = new DB_connect().DB_connect();
		result = statement.executeQuery("select id,user.userId,userName,signIn_time,signOut_time from user,sign where user.userId = sign.userId "
												+ "and left(signIn_time,10) = "+"'"+ date+"'"
